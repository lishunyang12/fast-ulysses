"""torchrun worker: an outstanding async call plus a sync call on another tag, streams unordered.
    ...
    torchrun --nproc_per_node=8 tests/distributed/a2a_overlapping_barriers.py
...
site in fast_ulysses/csrc/bindings.cpp (e.g. ``group->fast_barrier(stream, "")``), and rebuild.
