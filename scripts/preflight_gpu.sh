...
#
# CI has no GPU runner, so nothing in .github/ ever executes tests/test_multigpu.py -- the
...
# pass while the wheel is broken. GPU idleness is scripts/exclusive.sh's job, not this one's --
# numbers and pass/fail from a contended box are both worthless.
#
# Usage:
#   ./scripts/preflight_gpu.sh dist/fast_ulysses-0.1.0+torch213cu130-cp312-*.whl
#   GPUS=0,1,2,3 TORCH_INDEX=https://download.pytorch.org/whl/cu128 ./scripts/preflight_gpu.sh <whl>
#
# Environment:
...
set +e
(cd "${WORK}" && "${REPO}/scripts/exclusive.sh" "${GPUS}" -- \
    "${VENV}/bin/pytest" "${REPO}/tests/test_multigpu.py" -q) 2>&1 | tee "${suite_log}"
